...
os.chdir(r'C:\Users\abibeka\OneDrive - Kittelson & Associates, Inc\Documents\HSIP\DataMapping\April09-DataProcessing\ProcessedData')
...
x1 = pd.ExcelFile('CrashDataAggByHSIP_ProjApr-22-2020 19_30.xlsx')
x1.sheet_names
FunClass = x1.parse('FunctionalClassDat')
data= x1.parse('HSIP_Project_by_Year')
data.columns
...
data1.loc[:,CrashCols] = data1.loc[:,CrashCols].div(data1['Analysis Period'],axis=0)
...
data1.rename(columns={"HSIP Project ID":"HSIP_ProjID",
    'Improvement Type':"ImpType",
                      ...
                      "Method for Site Selection":"MethSiteSelection",
                      "Emphasis Area Related Improv. Type":"EmphArea", "Roadway Type":"RoadwayType",
                      "Urban/Rural":"UrbanRural","Cost Distribution":"CostDist",
...
data1.DISTRICT_N = data1.DISTRICT_N.astype("Int32").astype(str)
data1.DISTRICT_N = "District "+data1.DISTRICT_N
data1.groupby('DISTRICT_N').count()
data1.DISTRICT_N = pd.Categorical(data1.DISTRICT_N,
                                             ["District 1","District 2","District 3","District 4","District 5",
                                              "District 6","District 8","District 9","District 10","District 11",
                                              "District 12"],ordered=True)

# data1.DISTRICT_N.replace("District nan","NoData",inplace=True)
...
data1.columns
data1.ImpCat.value_counts()
data1.RoadwayType.value_counts()
data1.DISTRICT_N.value_counts()
data1.UrbanRural.value_counts()
data1.CostDist.value_counts()
data1.MethSiteSelection.value_counts()
...
data1_TotalCrash.groupby('UrbanRural')[['FatalSSI_Before','FatalSSI_After']].sum()
data1_TotalCrash.groupby('RoadwayType')[['FatalSSI_Before','FatalSSI_After']].sum()
data1_TotalCrash.groupby('CostDist')[['FatalSSI_Before','FatalSSI_After']].sum()
data1_TotalCrash.groupby('MethSiteSelection')[['FatalSSI_Before','FatalSSI_After']].sum()
data1_TotalCrash.groupby(['DISTRICT_N','EmphArea'])[['FatalSSI_Before','FatalSSI_After']].sum()
...
def GroupData(data1,data1_TotalCrash,CostCrash, Cat):
    data4 = data1.groupby(['Year',Cat])[['DollorSpentMil']].sum().round(2).reset_index()
    data1_tp = data1.groupby(['Year',Cat])[['ChangeInCrashes']].sum().round(2).reset_index()
    ...
    data7.loc[:,'BC'] = (data7.FatalSSI_CostT /data7.CostPerRow).round(2)
    data7 = data7.pivot(Cat,'Year','BC')
    return({"ChngCrash":data5,"Funds":data6,"BC":data7})


def HeatmapByCat(data,cbarLab,Ylab,SaveFig,dim=[10,10],cbar_ticks=""):
    ...
    if cbar_ticks=="":
        ...
        colPalette= sns.xkcd_palette(["pale red"]) +sns.color_palette('viridis_r')
        sns.heatmap(data, ax=ax3,cmap=colPalette,annot=True,linewidth=.5,fmt='g',
                  cbar_kws={'label':cbarLab , 'orientation': 'horizontal','ticks':cbar_ticks},vmin=-10,vmax=70)        
    ax3.set(ylabel =Ylab )
...
# data1_TotalCrash.DISTRICT_N = data1_TotalCrash.DISTRICT_N.cat.remove_categories('NoData')
data1_TotalCrash.DISTRICT_N.isna().sum()
data1.DISTRICT_N.isna().sum()
da_Tot = data1_TotalCrash.copy()
da_temp = data1.copy()
da_Tot = da_Tot[~da_Tot.DISTRICT_N.isna()]
da_temp = da_temp[~da_temp.DISTRICT_N.isna()]


for Cat1 in ['DISTRICT_N','PLANNING_P','ImpCat','EmphArea','UrbanRural','RoadwayType','CostDist','MethSiteSelection']:
    if Cat1=="DISTRICT_N":
...
def GroupCat3(da,CostCrash,Cat="EmphArea",CatValue = "Lane Departures"):
    ...
    da1 = da1[['HSIP_ProjID','Year','AnalysisPeriod','FatalSSI_Before','FatalSSI_After','ImpType','ImpCat','EmphArea','AADT','UrbanRural','RoadwayType','CostDist','MethSiteSelection',
                                                               'ChangeInFatal','ChangeInSSI','CostPerRow',"DISTRICT_N","PLANNING_P"]]
    da1.AADT.describe()
...
Cats= ['DollarSpent1000_Bin','EmphArea','ImpCat','AADT_Lab','UrbanRural','RoadwayType','DISTRICT_N','PLANNING_P']
...
for SaveNm, Dat in dataDict.items():
    ...
    for Cat in Cats:
        ...
        PlotlyScatter1(Dat,Cat,SaveNm=f"./StateWidePlots/{SaveNm}/Crash/{SaveNm}",Yvar="ChangeInFatalSSI",title1="Before-After Yearly Crash Rate",
                   hover_data1=     ['HSIP_ProjID','Year','FatalSSI_Before','FatalSSI_After','DISTRICT_N','PLANNING_P','AnalysisPeriod','ImpType','ImpCat','EmphArea','AADT','UrbanRural','RoadwayType','CostDist','MethSiteSelection',
        ...
        PlotlyScatter1(Dat,Cat,SaveNm=f"./StateWidePlots/{SaveNm}/BC/{SaveNm}",Yvar="BC",title1="Benfit Cost Ratio" ,
                       hover_data1=     ['HSIP_ProjID','Year','FatalSSI_Before','FatalSSI_After','DISTRICT_N','PLANNING_P',
                                         'AnalysisPeriod','ImpType','ImpCat','EmphArea','AADT','UrbanRural','RoadwayType','CostDist',
                                         'MethSiteSelection','ChangeInFatal','ChangeInSSI','DollorSpent1000','BC'])    



def GroupFilter_Cat(da1,CostCrash,filter1="EmphArea",filterVal = "Lane Departures",Cat2="ImpCat",colorSym="ImpType"):
    if Cat2!="DISTRICT_N":da1[Cat2].fillna("NoData",inplace=True)
...
Cats2= ['EmphArea','ImpCat','UrbanRural','RoadwayType','DISTRICT_N','PLANNING_P']
...
Cats2= ['DISTRICT_N','PLANNING_P','EmphArea','ImpCat','UrbanRural','RoadwayType']
for Var in Cats2:
    Cats_ = Cats2.copy()
    Cats_.remove(Var)
    if Var!= "DISTRICT_N": Cats_.remove('DISTRICT_N')
